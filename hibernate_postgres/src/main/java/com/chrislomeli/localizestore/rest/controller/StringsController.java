...
import com.chrislomeli.localizestore.repository.StringsRepository;
import com.chrislomeli.localizestore.rest.request.StringsRequest;
import com.chrislomeli.localizestore.rest.response.StringsResponse;
...
@RestController
@RequestMapping(StringsController.DRV)
public class StringsController {

    private static final ILogger LOGGER = SLoggerFactory.getLogger(StringsController.class);
    ...
    public static final String RESOURCE = "localize_strings";
    ...
    public StringsController(StringsRepository repository) {
            ...
            @RequestParam(name = "filter", defaultValue = "", required = false) final List<String> filters)
    {
        try {
            ...
            // transform
            var responses = teams.stream().map(StringsResponse::new).collect(Collectors.toList());
    ...
    @GetMapping(value = "/{id}")
    public ResponseEntity<StringsResponse> get(@PathVariable(value = "id") String id) {
        try {
            Optional<Strings> foundObject = repository.findById(id);
            if (foundObject.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            }
            return new ResponseEntity<StringsResponse>(new StringsResponse(foundObject.get()), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<StringsResponse>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @PostMapping("")
    public ResponseEntity<StringsResponse> create(@RequestBody StringsRequest request) {
        try {
            var t = repository.save(request.toRepository());
            return new ResponseEntity<>(new StringsResponse(t), HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<StringsResponse> update(@RequestBody StringsRequest request) {
        try {
            var t = repository.save(request.toRepository());
            return new ResponseEntity<>(new StringsResponse(t), HttpStatus.CREATED);
        } catch (Exception e) {
