...
import com.chrislomeli.localizestore.repository.TranslationRepository;
import com.chrislomeli.localizestore.rest.request.TranslationRequest;
import com.chrislomeli.localizestore.rest.response.TranslationResponse;
...
@RestController
@RequestMapping(TranslationController.DRV)
public class TranslationController {

    private static final ILogger LOGGER = SLoggerFactory.getLogger(TranslationController.class);
    ...
    public static final String RESOURCE = "localize_translations";
            ...
            @RequestParam(name = "filter", defaultValue = "", required = false) final List<String> filters)
    {
        try {
            ...
            // transform
            var responses = teams.stream().map(TranslationResponse::new).collect(Collectors.toList());
    ...
    @GetMapping(value = "/{id}")
    public ResponseEntity<TranslationResponse> get(@PathVariable(value = "id") String id) {
        try {
            Optional<Translation> foundObject = repository.findById(id);
            if (foundObject.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            }
            return new ResponseEntity<>(new TranslationResponse(foundObject.get()), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<TranslationResponse> create(@RequestBody TranslationRequest request) {
        try {
            ...
            return new ResponseEntity<>(new TranslationResponse(t), HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TranslationResponse> update(@RequestBody TranslationRequest request) {
        try {
            var t = repository.save(request.toRepository());
            return new ResponseEntity<>(new TranslationResponse(t), HttpStatus.CREATED);
        } catch (Exception e) {
