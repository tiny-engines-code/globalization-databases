...
import com.chrislomeli.localizestore.repository.DocumentRepository;
import com.chrislomeli.localizestore.rest.request.DocumentRequest;
import com.chrislomeli.localizestore.rest.response.DocumentResponse;
...
public class DocumentController {
            ...
            @RequestParam(name = "filter", defaultValue = "", required = false) final List<String> filters)
    {
        try {
            ...
            // transform
            var responses = teams.stream().map(DocumentResponse::new).collect(Collectors.toList());
    ...
    @GetMapping(value = "/{id}")
    public ResponseEntity<DocumentResponse> get(@PathVariable(value = "id") String id) {
        try {
            Optional<Document> foundObject = repository.findById(id);
            if (foundObject.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            }
            return new ResponseEntity<DocumentResponse>(new DocumentResponse(foundObject.get()), HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<DocumentResponse> create(@RequestBody DocumentRequest request) {
        try {
            ...
            return new ResponseEntity<>(new DocumentResponse(t), HttpStatus.CREATED);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
